/*
 * PLEngineSysNetworkAPI_Linux.cpp
 *
 *  Created on: 2009-11-19
 *      Author: Administrator
 */
#include "../header/Linux/PLEngineSysNetworkAPI_Linux.h"
...
//			}
	...
	HSockCollection __PLSysAPI_Net_CreateSockCollection(){
		PL_SockCollection * ptrColl;

		ptrColl = (PL_SockCollection *)PLSysAPI_Mem_TLSAllocateMemoryPool(PL_CompilerAllocSize(PL_SockCollection));
		if(ptrColl == NULL_PL)
			return NULL_PL;
		PLSysAPI_MemoryClearZero((void_pl *)ptrColl,PL_CompilerAllocSize(PL_SockCollection));
		FD_ZERO(&(ptrColl->set));
		return (HSockCollection)ptrColl;
	}
	HPL_Result __PLSysAPI_Net_AddSocketToCollection(HSockCollection hColl,HPLSocket hSK){
		PL_SockCollection * ptrColl;

		ptrColl = (PL_SockCollection *)hColl;
		if(ptrColl == NULL_PL)
			return HPLResult_Fail;
		if(hSK == Invalid_PLSocket)
			return HPLResult_Fail;
		FD_SET(hSK,&(ptrColl->set));
		return HPLResult_Fail;
	}
	HPL_Result __PLSysAPI_Net_RemoveSocketForCollection(HSockCollection hColl,HPLSocket hSK){
		PL_SockCollection * ptrColl;
		...
		if(ptrColl == NULL_PL)
			return HPLResult_Fail;
		FD_CLR(hSK,&(ptrColl->set));
		return HPLResult_Fail;
	}
	HPL_Result __PLSysAPI_Net_ClearSocketForCollection(HSockCollection hColl){
		PL_SockCollection * ptrColl;
	...
	intV_pl __PLSysAPI_Net_GetSockCollectionCurrentSize(HSockCollection hColl){

		return 0;
	}
	intV_pl __PLSysAPI_Net_GetSockCollectionMaxSize(HSockCollection hColl){

		return 0;
	}
	void_pl __PLSysAPI_Net_DestroyCollection(HSockCollection hColl){
		PL_SockCollection * ptrColl;
		ptrColl = (PL_SockCollection *)hColl;
		if(ptrColl != NULL_PL){
			FD_ZERO(&(ptrColl->set));
			PLSysAPI_Mem_FreeMemoryPool((void_pl *)ptrColl);
		}
	}
