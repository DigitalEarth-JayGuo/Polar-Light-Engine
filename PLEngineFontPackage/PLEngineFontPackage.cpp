 ...
 *      Author: DevLinux
 */
#include "PLEngineFontPackage.h"
#include "EngineFontAPI.h"
	PLLibraryHandle CreateLibraryPL(HPPLEngine hEngine,void_pl * ptrEx){
		PL_FontLibraryHeavy * ptrInface;
		PL_FontLibraryFunTable    * ptrTable;
	...
	//	char_pl pPath[PL3DSystemFileNameMaxLength];
		...
		void_pl * ptrConstCT;
		int iConstCTSize;
		if((hEngine == NULL_PL) || (g_hLibraryHandle_Font != NULL_PL))
			return NULL_PL;
		...
		if((ptrSysEnv == NULL_PL) || (ptrInfo == NULL_PL) || (hECTHandle == NULL_PL))
	 	...
	 	iInfaceSize = PL_CompilerAllocSize(PL_FontLibraryHeavy);
		ptrInface   = (PL_FontLibraryHeavy *)PLSysAPI_Mem_AllocateMemoryPool(iInfaceSize);
	...
	*/
	...
	void_pl Destory_LibraryPL(HPPLEngine hEngine,PLLibraryHandle hAndle,void_pl * ptrEx){
		...
		ptrInface = (PL_FontLibraryHeavy *)hAndle;
		if((hEngine == NULL_PL) || (hAndle == NULL_PL))
			return;
		g_hLibraryHandle_Font = NULL_PL;
		ptrSysEnv = PLSysAPI_Sys_GetOperatSystemEnv(hEngine);
		ptrInfo   = PLSysAPI_Sys_GetConstSystemInfo(hEngine);
