...
HMaterialMgrAndle Eg_CreateMaterialManager(HResDataSystem hRDSystem){
	PL_ResourceMaterialManager * ptrMgr;
	intV_pl iCount;
	ptrMgr = (PL_ResourceMaterialManager *)PLSysAPI_Mem_TLSAllocateMemoryPool(PL_CompilerAllocSize(PL_ResourceMaterialManager));
	PLSysAPI_MemoryClearZero(ptrMgr,PL_CompilerAllocSize(PL_ResourceMaterialManager));
	...
	for(iCount = 0;iCount<PLRMgr_ConstID_RMaterialMgrMapSlotMaxSize;iCount++){
		PLSysAPI_Syn_InitializeRWLockWF(ptrMgr->List[iCount].lock);
	}
	return (HMaterialMgrAndle)ptrMgr;
}
void_pl Eg_DestoryMaterialManager(HMaterialMgrAndle hAndle){
	intV_pl iCount;
	intV_pl iNCount;
	PL_ResourceMaterialManager * ptrMgr;
	PL_ResourceMaterialItem *    ptrItem;
	PL_ResourceMaterialItem *    ptrNItem;
	ptrMgr = (PL_ResourceMaterialManager *)hAndle;
	if(ptrMgr == NULL_PL)
		return;
	ptrMgr->iTkCount = 0;
	...
	for(iCount = 0;iCount < PLRMgr_ConstID_RMaterialMgrMapSlotMaxSize;iCount++){
		...
		while(ptrNItem!=NULL_PL){
			...
			__native_Eg_PushReclaimQueue<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr,ptrItem);
		}
	}

	while((ptrItem = __native_Eg_PopReclaimQueue<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr))!=NULL_PL){
		if(ptrItem->ptrName){
...
PL_ResourceMaterial * Eg_CreateMaterialMgrObject(HMaterialMgrAndle hAndle,const_pl char_pl * ptrKey,intV_pl iKeySize){
	PL_ResourceMaterialManager * ptrMgr;
	PL_ResourceMaterialItem *    ptrItem;
	PL_ResourceMaterialItem *    ptrNextItem;
	uintV_pl uHash;
	ptrMgr = (PL_ResourceMaterialManager *)hAndle;
	ptrItem = NULL_PL;
	uHash = ptrMgr->hashfunc(ptrKey,iKeySize,PLRMgr_ConstID_RMaterialMgrMapSlotMaxSize);
	...
	ptrItem = (PL_ResourceMaterialItem *)PLSysAPI_Mem_TLSAllocateMemoryPool(PL_CompilerAllocSize(PL_ResourceMaterialItem));
	if(ptrItem == NULL_PL)
		return NULL_PL;
	PLSysAPI_MemoryClearZero(ptrItem,PL_CompilerAllocSize(PL_ResourceMaterialItem));
	...
	ptrItem->eType = PLSys_Const_ResD_ePLMaterialMgr;
	ptrItem->iCount = 1;
	ptrItem->iRefCount = 0;
...
PL_ResourceMaterial * Eg_OpenMaterialMgrObject(HMaterialMgrAndle hAndle,const_pl char_pl * ptrKey,intV_pl iKeySize){
	PL_ResourceMaterialManager * ptrMgr;
	PL_ResourceMaterialItem *    ptrItem;
	PL_ResourceMaterialItem *    ptrNItem;
	PL_ResourceMaterialItem *    ptrRelaseItem;
	...
	if((ptrNItem != NULL_PL) && (ptrNItem->eDynamicState != PLSys_Const_RODynamic_eReclaim)){
		...
		if(iRefCount < 0){
			...
			if((eDynamicState != PLSys_Const_RODynamic_eReclaim) && (PLSysAPI_JudgeSetValueInt((volatile_pl intV_pl *)(&(ptrItem->eDynamicState)),
				...
				__native_Eg_RemoveMapItem<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr,uHash,ptrItem);
				ptrRelaseItem = ptrItem;
			}
			if(ptrRelaseItem != NULL_PL){
				__native_Eg_PushReclaimQueue<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr,ptrRelaseItem);
			}
			return NULL_PL;
		}
	}
	PLSysAPI_Syn_ReleaseRWLockReadWF(ptrMgr->List[uHash].lock);
	return (PL_ResourceMaterial *)ptrItem;
}
HPL_Result Eg_ReleaseMaterialMgrObject(PL_ResourceMaterial * ptrItem){
	PL_ResourceMaterialManager * ptrMgr;
	PL_ResourceMaterialItem * ptrPItem;
	PL_ResourceMaterialItem * ptrRelaseItem;
	PL_ResourceObjectDynamicState eDynamicState;
	intV_pl iRefCount;
	ptrPItem = (PL_ResourceMaterialItem *)ptrItem;
	uintV_pl uHash;
	ptrRelaseItem = NULL_PL;
	if((ptrPItem == NULL_PL) || (ptrPItem->hMgrAndle==NULL_PL))
		return NULL_PL;
	ptrMgr = (PL_ResourceMaterialManager *)(ptrPItem->hMgrAndle);
	uHash = ptrMgr->hashfunc(ptrPItem->ptrName,ptrPItem->iNSize,PLRMgr_ConstID_RMaterialMgrMapSlotMaxSize);
	PLSysAPI_Syn_AcquireRWLockWriteWF(ptrMgr->List[uHash].lock);
		if(ptrItem->eDynamicState != PLSys_Const_RODynamic_eReclaim){
			...
			if(iRefCount==0){
				...
				if((eDynamicState != PLSys_Const_RODynamic_eReclaim) && (PLSysAPI_JudgeSetValueInt((volatile_pl intV_pl *)(&(ptrPItem->eDynamicState)),
						(intV_pl)PLSys_Const_RODynamic_eReclaim,
						(intV_pl)eDynamicState)==true_pl)){
					__native_Eg_RemoveMapItem<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr,uHash,ptrPItem);
...
HPL_Result Eg_DestoryMaterialMgrObject(HMaterialMgrAndle hAndle,const_pl char_pl * ptrKey,intV_pl iKeySize){
	PL_ResourceMaterialManager * ptrMgr;
	PL_ResourceMaterialItem * ptrItem;
	uintV_pl uHash;
//	PL_ResourceObjectDynamicState eDynamicState;
	...
	uHash = ptrMgr->hashfunc(ptrKey,iKeySize,PLRMgr_ConstID_RMaterialMgrMapSlotMaxSize);
	PLSysAPI_Syn_AcquireRWLockWriteWF(ptrMgr->List[uHash].lock);
		ptrItem = __native_Eg_UnRefMapItem<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr,uHash,ptrKey,iKeySize);
	PLSysAPI_Syn_ReleaseRWLockWriteWF(ptrMgr->List[uHash].lock);
	if(ptrItem != NULL_PL){
		__native_Eg_PushReclaimQueue<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr,ptrItem);
	}
...
HPL_Result Eg_PushMaterialRTaskerQueue(PL_ResourceMaterial * ptrItem){
	PL_ResourceMaterialManager * ptrMgr;
	PL_ResourceMaterialItem * ptrPItem;
	PL_ResourceMaterialItem * ptrReleaseItem;
	intV_pl iRefCount;
	uintV_pl uHash;
	PL_ResourceObjectDynamicState eDynamicState;
	ptrPItem = (PL_ResourceMaterialItem *)ptrItem;
	...
	if(iRefCount<0){
		...
		if(ptrReleaseItem!=NULL_PL){
			__native_Eg_PushReclaimQueue<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr,ptrReleaseItem);
		}
		return HPLResult_Fail;
	}
	if(HResulPL_FAILED(PLSysAPI_Syn_TryEnterSpinLock(&(ptrMgr->TkLock))) == true_pl){
			...
			if(iRefCount == 0){
				PLSysAPI_Syn_AcquireRWLockWriteWF(ptrMgr->List[uHash].lock);
					...
					if((eDynamicState != PLSys_Const_RODynamic_eReclaim) && (PLSysAPI_JudgeSetValueInt((volatile_pl intV_pl *)(&(ptrPItem->eDynamicState)),
							(intV_pl)PLSys_Const_RODynamic_eReclaim,
							(intV_pl)eDynamicState)==true_pl)){
						ptrReleaseItem = __native_Eg_RemoveMapItem<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr,uHash,ptrPItem);
					}
				PLSysAPI_Syn_ReleaseRWLockWriteWF(ptrMgr->List[uHash].lock);
				if(ptrReleaseItem!=NULL_PL){
					__native_Eg_PushReclaimQueue<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr,ptrReleaseItem);
		...
		ptrPItem->ptrNext = (PL_ResourceMaterialItem *)(ptrMgr->ptrOverFlowQueueRoot);
		ptrMgr->ptrOverFlowQueueRoot = ptrPItem;
		PLSysAPI_Syn_LeaveSpinLock(&(ptrMgr->TkOvLock));
		return HPLResult_OK;
	}
	if(__native_Eg_PushPriorityStackSortList(ptrMgr,ptrPItem,PLRMgr_ConstID_RMaterialMgrTaskerListMaxL)==false_pl){
...
HPL_Result Eg_PushMaterialRTaskerQueueFill(PL_ResourceMaterial * ptrItem,PL_CommResourceBlock * ptrBlock){
	PL_ResourceMaterialManager * ptrMgr;
	PL_ResourceMaterialItem * ptrPItem;
	PL_ResourceMaterialItem * ptrReleaseItem;
	intV_pl iRefCount;
...
HPL_Result Eg_PushMaterialRTaskerQueueN(HMaterialMgrAndle hAndle,const_pl char_pl * ptrKey,intV_pl iKeySize){
	PL_ResourceMaterialManager * ptrMgr;
	PL_ResourceMaterialItem * ptrPItem;
	ptrMgr = (PL_ResourceMaterialManager *)hAndle;
	if(ptrMgr==NULL_PL)
		return HPLResult_Fail;
	ptrPItem = (PL_ResourceMaterialItem *)Eg_OpenMaterialMgrObject(hAndle,ptrKey,iKeySize);
	if(ptrPItem == NULL_PL)
		return HPLResult_Fail;
	if(PLSysAPI_JudgeSetValueInt((volatile_pl intV_pl *)(&(ptrPItem->eState)),
			(intV_pl)PLSys_Const_ResLoad_ePLBeginLoad,
			(intV_pl)PLSys_Const_ResLoad_ePLUnLoadComplete)==false_pl){
		Eg_ReleaseMaterialMgrObject((PL_ResourceMaterial *)ptrPItem);
		return HPLResult_Fail;
	}
	if(HResulPL_FAILED(PLSysAPI_Syn_TryEnterSpinLock(&(ptrMgr->TkLock))) == true_pl){
		if(HResulPL_FAILED(PLSysAPI_Syn_EnterSpinLock(&(ptrMgr->TkOvLock))) == true_pl){
...
HPL_Result Eg_PushMaterialRTaskerQueueNFill(HMaterialMgrAndle hAndle,const_pl char_pl * ptrKey,intV_pl iKeySize,PL_CommResourceBlock * ptrBlock){
	PL_ResourceMaterialManager * ptrMgr;
	PL_ResourceMaterialItem * ptrItem;
	ptrMgr = (PL_ResourceMaterialManager *)hAndle;
	...
	ptrItem = (PL_ResourceMaterialItem *)Eg_OpenPolygonMgrObject(hAndle,ptrKey,iKeySize);
	if(ptrItem == NULL_PL)
		return HPLResult_Fail;
...
HPL_Result Eg_RefurbishMaterialRBackgroundDispose(HMaterialMgrAndle hAndle,PL_ResourceBgDisposeParamer * ptrParamer){
	PL_ResourceMaterialManager * ptrMgr;
	PL_ResourceMaterialItem * ptrItem;
	PL_ResourceMaterialItem * ptrRelaseItem;
	PL_ResourceObjectDynamicState eDynamicState;
	...
	if((ptrMgr == NULL_PL) || (ptrParamer == NULL_PL))
		return HPLResult_Fail;
	ptrRelaseItem = NULL_PL;
	if((ptrMgr->ptrOverFlowQueueRoot != NULL_PL) && (ptrMgr->iTkCount<PLRMgr_ConstID_RMaterialMgrTaskerListMaxL)){
		ptrItem = NULL_PL;
		if(HResulPL_FAILED(PLSysAPI_Syn_EnterSpinLock(&(ptrMgr->TkOvLock))) == false_pl){
			if(ptrMgr->ptrOverFlowQueueRoot!=NULL_PL){
				ptrItem = (PL_ResourceMaterialItem *)(ptrMgr->ptrOverFlowQueueRoot);
		...
		if(ptrItem != NULL_PL){
			if(HResulPL_FAILED(PLSysAPI_Syn_EnterSpinLock(&(ptrMgr->TkLock))) == false_pl){
				if(__native_Eg_PushPriorityStackSortList(ptrMgr,ptrItem,PLRMgr_ConstID_RMaterialMgrTaskerListMaxL)==false_pl){
					if(HResulPL_FAILED(PLSysAPI_Syn_EnterSpinLock(&(ptrMgr->TkOvLock))) == true_pl){
						...
						ptrItem->ptrNext = (PL_ResourceMaterialItem *)(ptrMgr->ptrOverFlowQueueRoot);
	...
	if(ptrMgr->iTkCount>0){
		if(HResulPL_FAILED(PLSysAPI_Syn_EnterSpinLock(&(ptrMgr->TkLock)))==false_pl){
			ptrItem = (PL_ResourceMaterialItem *)(__native_Eg_PopMaxPriorityStackSort<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr));
			PLSysAPI_Syn_LeaveSpinLock(&(ptrMgr->TkLock));
			if(ptrItem != NULL_PL){
	...
	if(ptrMgr->ptrReclaimQueueRoot != NULL_PL){
		while((ptrItem = (PL_ResourceMaterialItem *)__native_Eg_PopReclaimQueue<PL_ResourceMaterialManager,PL_ResourceMaterialItem>(ptrMgr))!=NULL_PL){
