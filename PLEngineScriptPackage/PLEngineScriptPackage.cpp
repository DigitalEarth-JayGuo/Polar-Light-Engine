 ...
 */
#include "PLEngineScriptPackage.h"
...
//		PLFileCFGHandle hCFGHandle;
		...
		ptrInface   = (PL_ScriptLibraryHeavy *)PLSysAPI_Mem_AllocateMemoryPool(iInfaceSize);
		if(ptrInface==NULL_PL){

			return NULL_PL;
		}
		PLSysAPI_MemoryClearZero((void_pl *)(ptrInface),iInfaceSize);
		iTableSize = PL_CompilerAllocSize(PL_ScriptLibraryFunTable) + sizeof(fun_pl) * PLScript_Table_FunMaxSize;
		ptrTable   = (PL_ScriptLibraryFunTable *)PLSysAPI_Mem_AllocateMemoryPool(iTableSize);
		if(ptrTable == NULL_PL){
			if(ptrInface!=NULL_PL)
